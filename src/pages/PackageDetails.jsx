...
//   return (
//     <div className="min-h-screen bg-offwhite py-12 sm:py-16 lg:py-20 overflow-x-hidden">
...
//                 </div>
//               ) : (
//                 <div className="aspect-square sm:aspect-[4/3] lg:aspect-square bg-forest/10 rounded-xl sm:rounded-2xl flex items-center justify-center">
//                   <div className="text-center">
...
//                       <div
//                         key={type.id}
//                         className="flex items-center gap-1 sm:gap-2 bg-earthy/10 px-2 sm:px-3 py-2 rounded-lg border border-earthy/20 min-w-0 flex-shrink-0"
//                       >
//                         <span className="material-symbols-outlined text-earthy text-sm sm:text-base flex-shrink-0">home</span>
//                         <span className="font-medium text-earthy text-sm sm:text-base truncate">{type.name}</span>
//                       </div>
//                     ))}
//                   </div>
//                 </div>
...
//                 className="block w-full bg-primary text-white text-center py-3 sm:py-4 px-4 sm:px-6 rounded-xl font-bold text-base sm:text-lg hover:bg-primary/90 transition-all transform hover:scale-[1.02] active:scale-[0.98] shadow-lg"
//               >
...
export default function PackageDetails() {
  ...
  useEffect(() => {
    const fetchPackageDetails = async () => {
      try {
        ...
        setPackageData(
          packageItem ? normalizePackageItems(packageItem) : null
        );
      } catch (err) {
        console.error("Failed to fetch package details:", err);
        setError(err.message);
  ...
  const nextImage = () => {
    if (packageData?.images?.length) {
      setCurrentImageIndex(
        (prev) => (prev + 1) % packageData.images.length
      );
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-offwhite py-20 overflow-x-hidden">
        <div className="container mx-auto px-4">
          <div className="animate-pulse">
            <div className="h-8 bg-slate-700 rounded w-1/3 mb-8"></div>
            <div className="grid md:grid-cols-2 gap-8">
  ...
  if (error || !packageData) {
    return (
      <div className="min-h-screen bg-offwhite flex items-center justify-center overflow-x-hidden">
  ...
  return (
    <div className="min-h-screen bg-offwhite py-12 sm:py-16 lg:py-20 overflow-x-hidden">
      <div className="container mx-auto px-3 sm:px-4 md:px-6 max-w-7xl">
        ...
        <div className="grid md:grid-cols-2 gap-6 lg:gap-12">
          {/* Image Gallery - fixed 800x800-like square */}
          <Reveal variant="fade-up" delay={200}>
            <div className="relative flex flex-col items-center">
              {packageData.images?.length > 0 ? (
                <div className="relative w-full max-w-[800px] rounded-xl sm:rounded-2xl overflow-hidden bg-forest/5">
                  {/* Constant 1:1 frame, interpreted as 800x800 */}
                  <div className="relative w-full aspect-square">
                    <img
                      src={
                        packageData.images[currentImageIndex]?.image_url
                      }
                      alt={
                        packageData.images[currentImageIndex]?.caption ||
                        packageData.name
                    ...
                    {packageData.images.length > 1 && (
                      <>
                        <button
                          ...
                          className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 bg-white/80 backdrop-blur-sm rounded-full p-1.5 sm:p-2 hover:bg-white transition-colors shadow-lg"
                        >
                          <span className="material-symbols-outlined text-forest text-lg sm:text-xl">
                            chevron_left
                          </span>
                        </button>
                        <button
                ...
                <div className="relative w-full max-w-[800px] rounded-xl sm:rounded-2xl overflow-hidden bg-forest/10">
                  <div className="relative w-full aspect-square flex items-center justify-center">
                    <div className="text-center">
                      <span className="material-symbols-outlined text-forest/30 text-4xl sm:text-6xl mb-4">
                        home
                      </span>
                      <p className="text-forest/50 text-sm sm:text-base">
                        No images available
                      </p>
                    </div>
                  </div>
              ...
              {/* Thumbnail Strip */}
              {packageData.images?.length > 1 && (
                <div className="flex gap-1.5 sm:gap-2 mt-3 sm:mt-4 overflow-x-auto pb-2 w-full max-w-[800px]">
                  {packageData.images.map((img, index) => (
                    <button
                      key={img.id ?? index}
                      onClick={() => setCurrentImageIndex(index)}
          ...
          <div className="space-y-6 sm:space-y-8">
            ...
            <Reveal variant="fade-up" delay={300}>
              <div>
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black text-forest mb-4 break-words">
                  {packageData.name} Package
                </h1>
                <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6 text-earthy text-sm sm:text-base">
                  <div className="flex items-center gap-2">
            ...
            <Reveal variant="fade-up" delay={400}>
              <div className="bg-gradient-to-r from-primary/10 to-earthy/10 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-primary/20">
                <div className="flex items-baseline gap-2">
                  ...
                  <span className="text-earthy/70 text-sm sm:text-base">
                    per person
                  </span>
                </div>
                {packageData.promo_active && (
            ...
            <Reveal variant="fade-up" delay={500}>
              <div>
                <h2 className="text-xl sm:text-2xl font-bold text-forest mb-4 sm:mb-6">
                  What's Included
                </h2>
                <div className="space-y-2 sm:space-y-3">
                  {features.map((feature, index) => (
                    <div
                      ...
                      className="flex items-start gap-3 p-3 sm:p-4 rounded-lg hover:bg-forest/5 transition-colors"
                    >
                      <span className="material-symbols-outlined text-emerald-600 text-lg sm:text-xl flex-shrink-0 mt-0.5">
                        check_circle
                      </span>
                      <span className="text-forest/80 leading-relaxed text-sm sm:text-base break-words">
                        {feature}
                      </span>
                    </div>
                  ))}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 sm:gap-3">
                    {packageData.allowed_unit_types.map((type) => (
                      <div
                        key={type.id}
                        className="flex items-center gap-1 sm:gap-2 bg-earthy/10 px-2 sm:px-3 py-2 rounded-lg border border-earthy/20 max-w-full"
                      >
                        <span className="material-symbols-outlined text-earthy text-sm sm:text-base flex-shrink-0">
                          home
                        </span>
                        <span className="font-medium text-earthy text-sm sm:text-base break-words">
                          {type.name}
                        </span>
                      </div>
                    ))}
            ...
            <Reveal variant="fade-up" delay={700}>
              <div className="bg-forest/5 rounded-xl p-4 sm:p-6">
                <h3 className="font-bold text-forest mb-3 text-lg sm:text-xl">
                  Child Policy
                </h3>
                <div className="space-y-2 text-forest/80 text-sm sm:text-base">
                  <p>
                    • Children under {packageData.child_free_max_age} years:
                    Free
                  </p>
                  <p>
                    • Children between{" "}
                    {packageData.child_free_max_age + 1}-
                    {packageData.child_half_max_age} years: 50% charge
                  </p>
                  <p>
                    • Children above {packageData.child_half_max_age} years:
                    Full charge
                  </p>
                </div>
              </div>
